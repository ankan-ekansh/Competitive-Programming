...
#define ll long long
bool comp(pair<ll, ull> a, pair<ull, ll> b){
    if(a.first != b.first){
    ...
    while(t--){
        ...
        for(ull i=0;i<n;i++){
            ...
            if(a[i] < (a[i] ^ x)){
                lc++;
                //lows.push_back(a[i]);
                diff.push_back(make_pair((a[i]^x) - a[i], a[i]^x));
                allHighSum+=(a[i] ^ x);
            }
            else if(a[i] > (a[i] ^ x)){
                hc++;
                //highs.push_back(a[i] ^ x);
                diff.push_back(make_pair(a[i] - (a[i]^x), a[i]));
                allHighSum+=a[i];
            }
        }
        ...
        if(lc % 2 != 0){
            if(hc % 2 != 0){
                if(k % 2 != 0){
                    ...
                    //oneLowAllHighSum = solve(a, n, l, x);
                    // cout<<"High sum ="<<allHighSum<<"\n";
                    // cout<<"L = "<<l<<"\n";
                    oneLowAllHighSum = allHighSum - l + (l^x);
                    cout<<oneLowAllHighSum<<"\n";
                    continue;
                }
            }
